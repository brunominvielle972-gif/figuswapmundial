...
export default function MyTrades({ onNavigateToFriends }: MyTradesProps) {
  ...
  const getInviteUrl = () => {
    ...
    return `${origin}?invite=${currentUser.uid}`;
  };
          ...
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <span className="text-[9px] text-slate-350 block">Hacé clic abajo para abrir o copialo para pasárselo a tu amigo por WhatsApp:</span>
              </div>
            </div>
            ...
            >
              <Copy className="w-3.5 h-3.5" /> {copiedLink ? "¡Copiado!" : "Copiar"}
            </button>
          </div>
        ...
        {activeTrade ? (
          ...
          <div className="flex-1 flex flex-col justify-center items-center text-slate-500 text-sm gap-2 p-8 text-center bg-brand-bg/20">
            ...
            <div className="mt-6 p-4.5 bg-blue-950/20 border border-blue-500/25 rounded-2xl max-w-sm flex flex-col items-center gap-2.5">
              <span className="text-[10.5px] font-black text-blue-300 uppercase tracking-widest block text-center">¿Buscás más compañeros de canje?</span>
              <p className="text-[10px] text-slate-350 text-center leading-relaxed">
                ¡Enviá este enlace a tus amigos! Podrán tocarlo directamente para conectarse e intercambiar figuritas en el acto:
              </p>
              ...
              <div className="w-full bg-[#030a16] px-3 py-2.5 rounded-xl border border-blue-500/20 flex flex-col gap-2.5 items-center overflow-hidden">
                ...
                <button
                  type="button"
                  onClick={handleCopyLink}
                  className={`px-3 py-2 font-mono font-black text-[9.5px] uppercase rounded-lg transition-all flex items-center gap-1.5 cursor-pointer border ${
                    copiedLink
                ...
                >
                  <Copy className="w-3.5 h-3.5" /> {copiedLink ? "¡Enlace Copiado!" : "Copiar Enlace"}
                </button>
              </div>
            </div>
          </div>
